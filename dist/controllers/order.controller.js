...
export const hasPurchasedOrNot = asyncHandler(async (req, res, next) => {
    const { id } = req.params;
    const orders = await Order.find({
        userId: req.user._id,
        orderStatus: "delivered",
    });
    const hasPurchasedOrNot = orders.some((order) => order.orderItems.some((item) => item.productId.toString() === id.toString()));
    return res.status(200).json(new ApiResponse(200, {
        hasPurchasedOrNot,
    }));
});
